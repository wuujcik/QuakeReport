...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            //Create JSONObject from SAMPLE_JSON_RESPONSE string
            JSONObject baseJsonResponse = new JSONObject(SAMPLE_JSON_RESPONSE);

            //Extract the JSONArray associated with key "features"
            // that represents a list of earthquakes
            JSONArray earthquakeArray = baseJsonResponse.getJSONArray("features");

            //For each earthquakeArray, create an {@link Earthquake} object
            for (int i = 0; i < earthquakeArray.length() ; i++){
                //get currentQuakeObject
                JSONObject currentQuakeObject = earthquakeArray.getJSONObject(i);
                //extract from the currentQuakeObject a JSONObject associated with key "properties"
                JSONObject properties = currentQuakeObject.getJSONObject("properties");

                //Extract the values from properties for the key: magnitude, location and time
                ...
                earthquakes.add(new Earthquake(location, magnitude, time, web));
            }
